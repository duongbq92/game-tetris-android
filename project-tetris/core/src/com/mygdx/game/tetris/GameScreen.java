...
public class GameScreen
{
	...
	public GameScreen(int width, int height)
	{
		...
		Gdx.app.log("DEBUG - Width", screenWidth + "");
		Gdx.app.log("DEBUG - Height", screenHeight + "");
		...
		/*
		 * blackDigitFont = new
		 * BitmapFont(Gdx.files.internal("data/BlackDigit.fnt"),
		...
		blackRecTexture = new Texture(Gdx.files.internal("data/blackrec.png"));
		blackRecTexture.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		// digitNumber.ChangeNumberTo(100);

		arialFont = new BitmapFont(Gdx.files.internal("data/BlackArial30.fnt"),
				Gdx.files.internal("data/BlackArial30.png"), true);
		whiteArialFont40 = new BitmapFont(
				Gdx.files.internal("data/WhiteArial40.fnt"),
				Gdx.files.internal("data/WhiteArial40.png"), true);
		...
		speedDigitNumber = new DigitNumber(2, nextBrickX + 25, nextBrickY + 4
				* (CELL_OFFSET + CELL_SIZE) + 60, false);
		levelDigitNumber = new DigitNumber(2, nextBrickX + 25, nextBrickY + 4
				* (CELL_OFFSET + CELL_SIZE) + 110, false);
		...
		listOfNormalBricks.add(new BrickType9());
		 ...
		 * System.out.println(a.width); System.out.println(a.height);
		 */
		...
		myKeyDownHandler.KeyDown = new IKeyDownHandler()
		{
			...
			public void KeyDownHandler(int keycode)
			{
				...
				{
					delayInputCount++;
					//if (delayInputCount % 2 ==0) {
						if (keycode == Keys.LEFT && !CurrentBrick.IsCheckingScore) {
							CurrentBrick.Go(0);
						} else if (keycode == Keys.RIGHT
								&& !CurrentBrick.IsCheckingScore) {
							CurrentBrick.Go(1);
						}
						if (keycode == Keys.DOWN) {
							if (!CurrentBrick.Go(2)
									&& !CurrentBrick.IsCheckingScore)// prevent
							// player
							// press
							// down
							// while
							 ...
							 */
								...
								delayCount = Player.CurrentSpeedDelay;// modify
								// delayCount
								// so it
								// will
								// check
		...
		myKeyDownHandler.KeyDownOneTime = new IKeyDownHandler()
		{
			...
			public void KeyDownHandler(int keycode)
			{
				if (keycode == Keys.UP && !CurrentBrick.IsCheckingScore && !isPausingGame)
				{
					CurrentBrick.Rotate();
				}
			}
	...
	private void CreateNewBreak()
	{
		...
		if (CurrentBrick == null)
		{
			...
			CurrentBrick = listOfNormalBricks.get(ran_num).Clone();

			ran_num = GlobalRandom.nextInt(listOfNormalBricks.size());
			NextBrick = listOfNormalBricks.get(ran_num).Clone();
		...
		} else
		{
			...
			if (!CurrentBrick.Init())
			{
				...
				{
					showHighScore = true;
					MyGdxGame.iActivityRequestHandler.getLeaderboardGPGS();
					MyGdxGame.iActivityRequestHandler.submitScoreGPGS(Player.CurrentScore);
			...
			{
				if (GlobalRandom.nextDouble() < 0.05)// the rate of appearance
														// of BrickType8 is 5%
				{
				...
				{
					ran_num = GlobalRandom.nextInt(listOfNormalBricks.size());// except
																				// the
																				// BrickType8
					NextBrick = listOfNormalBricks.get(ran_num).Clone();
				}
	...
	public void Draw(SpriteBatch batch)
	{
		...
		// draw vertical line
		for (int i = 0; i < NUM_VERTICAL_CELLS; i++)
			batch.draw(blackLineTexture, NUM_HORIZONTAL_CELLS
					* (CELL_SIZE + CELL_OFFSET), i * (CELL_SIZE + CELL_OFFSET),
					4, CELL_SIZE + CELL_OFFSET, 0, 0, 4, 30, false, false);
		...
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				if (next_array[i][j] == 0)
				{
					batch.setColor(1, 1, 1, 0.7f);
					batch.draw(emptycellTexture, j * (CELL_SIZE + CELL_OFFSET)
							+ nextBrickX, i * (CELL_SIZE + CELL_OFFSET)
							+ nextBrickY, CELL_SIZE, CELL_SIZE, 0, 0,
							CELL_SIZE, CELL_SIZE, false, false);
				} else
				{
					// filled
					batch.setColor(1, 1, 1, 1);
					batch.draw(filledcellTexture, j * (CELL_SIZE + CELL_OFFSET)
							+ nextBrickX, i * (CELL_SIZE + CELL_OFFSET)
							+ nextBrickY, CELL_SIZE, CELL_SIZE, 0, 0,
							CELL_SIZE, CELL_SIZE, false, false);
				}
		...
		arialFont.draw(batch, "SPEED", nextBrickX + 15, nextBrickY + 4
				* (CELL_SIZE + CELL_OFFSET) + 5);
		...
		batch.draw(blackRecTexture, nextBrickX, screenHeight - 50, 120, 31, 0,
				0, 128, 32, false, false);

		whiteArialFont40.setColor(156 / 255.0f, 178 / 255.0f, 165 / 255.0f, 1);

		whiteArialFont40
				.draw(batch, "PAUSE", nextBrickX + 3, screenHeight - 48);
		/*
		 * blackDigitFont.setColor(1, 1, 1, 0.15f); blackDigitFont.draw(batch,
		 * "000000", 0, 70);
		 * 
		 * //draw score text blackDigitFont.setColor(1, 1, 1, 1);
